#!/usr/bin/env node
/* patch_posicionar.js — insere NEREAL_PATCH_POSICIONAR_NOS_V1 no engine.
   Uso: node patch_posicionar.js [--status|--dry-run|--apply|--rollback] arch_neural_v15_final.js
   Valida sintaxe (node --check) ANTES e DEPOIS; se quebrar, restaura backup.
   Bloco lido de posicionar_nos.js (mesma pasta). */
'use strict';
const fs = require('fs');
const path = require('path');
const { execSync } = require('child_process');

const SENT_INI = 'NEREAL_PATCH_POSICIONAR_NOS_V1';
const SENT_FIM = 'FIM NEREAL_PATCH_POSICIONAR_NOS_V1';
const args = process.argv.slice(2);
const mode = (args.find(a => a.startsWith('--')) || '--status').replace('--','');
const alvo = args.find(a => !a.startsWith('--')) || 'arch_neural_v15_final.js';
const blocoPath = path.join(__dirname, 'posicionar_nos.js');
