...
function v112_importar_merge(s){
  ...
  for(const n_in of (s.nodes || [])){
    if(n_in.text && por_texto_atual[n_in.text]){
      ...
      const igual = V112.nodes.find(x => x.camada === n_in.camada && x.text === n_in.text);
  ...
  for(const e of (s.edges || [])){
    ...
    const existente = V112.edges.find(x => x.from === from_new && x.to === to_new && x.tipo === (e.tipo || 'normal'));
    if(existente){
  ...
  function merge_sr(nome_sr, campos){
    ...
    const node_atual = V112.nodes.find(x => x.id === V112.subredes[nome_sr].id);
